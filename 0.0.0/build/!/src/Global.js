...
function Run () {
	var contents = []
	fs.readdirSync(path.resolve(global.buildConfigs.dirs.src, 'Global'), {withFileTypes: true}).forEach(function (entry) {
		if (entry.isFile() && !entry.name.startsWith('_') && path.extname(entry.name) == '.js') {
			contents.push(Minify(path.resolve(global.buildConfigs.dirs.src, 'Global', entry.name)))
		}
	})
	contents = uglifyjs.minify('+function () {\n\t'+contents.join('\n\t')+'\n}()', {compress : {keep_fnames: true, dead_code: false}, mangle: true}).code
	fs.writeFileSync(path.resolve(global.buildConfigs.dirs.dist, 'src', 'Global.js'), contents, 'utf8')
	return contents	
}
