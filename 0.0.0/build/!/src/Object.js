...
function Run () {
	var contents = []
	fs.readdirSync(path.resolve(global.buildConfigs.dirs.src, 'Object'), {withFileTypes: true}).forEach(function (entry) {
		if (entry.isFile() && !entry.name.startsWith('_') && path.extname(entry.name) == '.js') {
			contents.push(Minify(path.resolve(global.buildConfigs.dirs.src, 'Object', entry.name)))
	...
	fs.writeFileSync(path.resolve(global.buildConfigs.dirs.dist, 'src', 'Object.js'), contents, 'utf8')
	return contents	
}
